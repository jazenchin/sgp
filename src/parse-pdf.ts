...
type ParseMode = "anthropic" | "cli" | "api";
...
async function parseByAnthropic(prompt: string): Promise<unknown> {
  const client = new Anthropic();
  const message = await client.messages.create({
    model: "claude-haiku-4-5-20251001",
    max_tokens: 4096,
    system: "你只輸出合法 JSON，不使用 markdown code block，不輸出任何額外說明。",
    messages: [{ role: "user", content: prompt }]
  });
  const block = message.content[0];
  if (block.type !== "text") throw new Error("Unexpected Anthropic response type");
  return JSON.parse(cleanJsonResponse(block.text));
}
...
async function parseByCli(promptPath: string): Promise<unknown> {
  ...
  return JSON.parse(cleanJsonResponse(stdout));
}
...
export async function parsePdfFile(pdfPath: string, mode?: ParseMode) {
  try {
    const buf = await readFile(pdfPath);
    const result = await pdf(buf);
    const text = result.text.replace(/\r/g, "");
    const code = path.basename(pdfPath).match(/(\d{4,6})/)?.[1] ?? "unknown";
    ...
    // Explicit PDF_PARSE_MODE overrides everything
    const explicitMode = mode ?? (process.env.PDF_PARSE_MODE as ParseMode | undefined);

    let llmExtraction: unknown = null;
    const warnings: string[] = [];

    if (explicitMode === "anthropic") {
      llmExtraction = await parseByAnthropic(prompt);
    } else if (explicitMode === "api") {
      llmExtraction = await parseByApi(prompt);
    } else {
      // Default: try codex CLI first, fallback to Anthropic API, then generic API
      try {
        llmExtraction = await parseByCli(promptPath);
      } catch (cliError) {
        warnings.push(`codex CLI failed: ${String(cliError)}`);
        if (process.env.ANTHROPIC_API_KEY) {
          try {
            llmExtraction = await parseByAnthropic(prompt);
          } catch (apiError) {
            warnings.push(`Anthropic API fallback failed: ${String(apiError)}`);
          }
        } else if (process.env.LLM_API_ENDPOINT) {
          try {
            llmExtraction = await parseByApi(prompt);
          } catch (apiError) {
            warnings.push(`LLM API fallback failed: ${String(apiError)}`);
          }
        }
      }
    }

    if (warnings.length && !llmExtraction) {
      llmExtraction = { extraction: { warnings, confidence: 0 } };
    }

    return {
      llmExtraction,
      rawTextHash: createHash("sha256").update(text).digest("hex"),
      needsReview: warnings.length > 0 && !llmExtraction
    };
  } catch {
